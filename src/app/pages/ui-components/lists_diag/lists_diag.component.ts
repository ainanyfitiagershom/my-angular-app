...
@Component({
  ...
  templateUrl: './lists_diag.component.html',
})
export class AppListsDiagComponent implements OnInit {
  displayedColumns1: string[] = ['voiture', 'mecanicien', 'observation', 'date_diag', 'etat', 'action'];
  ...
  obtenirDiagnostics() {
    ...
    this.diagnosticService.getDiagnosticsByClient(user._id).subscribe(
      (data) => {
        this.dataSource1 = data.map((diag: any) => ({
          id: diag._id,
          voiture: `${diag.voiture.model.name} - ${diag.voiture.energie.nom} (${diag.voiture.transmission.nom})`,
          mecanicien: diag.mecanicien.nom,
