...
export class AppListsDiagMecanicienComponent implements OnInit {
  ...
  obtenirDiagnostics() {
    ...
    this.diagnosticService.getDiagnosticsByMecanicien(user._id).subscribe(
      (data) => {
  ...
  fairediag(id: string) {
    this.router.navigate(['ui-components/diagnostic', id]); // Redirige vers la page /diagnostic/{id}
  }
